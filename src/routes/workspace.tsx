        ...
        {tab === "Sales Rep" ? (
          <Section title="Today's priority deals" description="Tap Fix with AI to update a record in seconds.">
            <div className="grid gap-3">
              {deals.map((d) => (
                <DealRow key={d.id} deal={d} primary={d.id === "acme"} />
              ))}
            </div>
          </Section>
...
function DealRow({ deal, primary }: { deal: Deal; primary?: boolean }) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-col items-stretch gap-2 min-w-[160px]">
          {primary && !isUpdated ? (
            ...
            <button className="inline-flex items-center justify-center gap-2 rounded-md border border-border bg-background px-4 py-2 text-sm font-medium text-foreground hover:bg-muted">
